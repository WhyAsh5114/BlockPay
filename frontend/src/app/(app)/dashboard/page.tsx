...
async function fetchTokenPrice(
  network: keyof typeof NETWORK_COIN_IDS
): Promise<number> {
  try {
...
async function convertGweiToUSD(
  gwei: bigint,
...
function GasPriceCard({
  ...
  useEffect(() => {
    if (data?.maxFeePerGas) {
      const gasLimit =
        DEFAULT_GAS_LIMITS[name as keyof typeof DEFAULT_GAS_LIMITS];
      convertGweiToUSD(
        data.maxFeePerGas,
        gasLimit,
  ...
  return (
    <Card className="group transition-all duration-200 hover:scale-105 bg-gray-800">
      <CardHeader
        className={`${NETWORK_STYLES[name as keyof typeof NETWORK_STYLES]} text-white rounded-t-lg`}
      >
...
function GasPriceComparison() {
  ...
  const [sortedNetworks, setSortedNetworks] = useState<
    Array<{ name: string; data: any; color: string; usdCost?: number }>
  >([]);
    ...
    Promise.all(
      networks.map(async (network) => {
        if (network.data?.maxFeePerGas) {
          const gasLimit =
            DEFAULT_GAS_LIMITS[network.name as keyof typeof DEFAULT_GAS_LIMITS];
